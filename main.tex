...
\mydef{Outlier Detection}{
    The process of identifying data points that deviate from the normal data. The challenge is in defining what normal means.
}
...
\minititle{Why Does Outlier Detection Matter?}
...
\mydef{Types of Outliers}{
    Point, Contextual, and Collective Outliers.
}

\mydef{Point Outliers}{
    A single data point deviates significantly from all others. Example: A sudden temperature spike in a weather dataset.
}

\mydef{Contextual Outliers}{
    A value that is only abnormal in a given context or condition. Example: A warm winter day may be abnormal for the season, even if the temperature itself isn’t extreme.
}

\mydef{Collective Outliers}{
    A group of data points that together behave unusually. Example: A sequence of failed login attempts from a single IP address.
}

\mydef{Global vs. Local Outliers}{
    An outlier can be \textbf{global} if it is relative to the entire dataset. If it is within its local neighborhood or cluster, then it is \textbf{local}. Example: A data point might blend into the dataset globally but still stand out when compared to nearby points.
}

\minititle{Challenges in Outlier Detection}
\begin{itemize}
...
\minititle{Outlier Detection Techniques (Overview)}
\begin{itemize}
...
\mydef{Evaluating Outlier Detection Methods}{
    Precision at k (P@k) and Average Precision (AP)
}
...
\commandnote{
\begin{itemize}
  \item Max $P@k$ is the best possible precision achievable at each $k$ if all outliers are ranked at the top.
  \item Actual $P@k$ depends on your model’s output. It can match max $P@k$ if your model ranks perfectly.
\end{itemize}
}

\mydef{Expected Value under random ordering}{
The expected value of P@k under random ordering is what we expect just by chance, i.e. if the ranking is random:
}
\[
...
\mydef{Adjusted P@k}{
It tells us how much better our model's ranking is compared to random guessing, scaled relative to the best possible performance.
}
\[
...
\minititle{Expected Value of AP under random ranking}
\[
...
\minititle{Isolation Tree Construction Algorithm}
...
\minititle{Example: Building an Isolation Tree (2D Data)}
...
\minititle{Problem: Incomplete Isolation}
...
\minititle{Solution: Approximating Path Length \(\ell_T(x)\)}
...
\minititle{Very Simple Example (1D)}
...
\mydef{k-Distance}{
    Let $x \in X$. Sort all other points by distance to $x$: $y_1, y_2, \cdots, y_{n-1}$ such that $d(x,y_1) \leq d(x, y_2) \leq \cdots$. Then define $d_k(x)=d(x,y_k)$ as the distance from $x$ to its $k^{\text{th}}$ nearest neighbor.
}

\mydef{K-Neighborhood}{
    Define $N_k(x) = \set{y \in X \mid d(x, y) \leq d_k(x)}$ as the set of points that lie within distance $d_k(x)$
}
...
\minititle{The Non-Symmetric Behavior}
...
\minititle{Local Reachability Density (LRD)}
...
\minititle{Max and Min Reachability Distance in a Cluster}
...
\minititle{Relative k-reachability distance spread}
...
\minititle{Two-hop neighborhood}
...
\minititle{Bounding $\text{LOF}_k(x)$}
...
\newpage
\minititle{Potential Exercise Discussion}
